...
export type ReactiveConfig<T> = (BaseConfig<T> & {
    type: "signal";
}) | {
    type: "memo";
    compute: (deps: any) => T;
    ...
    type: "resource";
    fetcher: (source: any, { value, refetching }: any) => T | Promise<T>;
    source?: any;
...
export type ComponentContext<T extends ComponentFactory> = {
    ...
    addReactive: <K extends string>(key: K, config: ReactiveConfig<any>) => void;
    removeReactive: (key: keyof T) => void;
    reset: () => void;
    subscribe: (key: keyof T, callback: (value: any) => void) => () => void;
};
export declare function createComponentFactory<T extends ComponentFactory>(config: T): ComponentContext<T>;
